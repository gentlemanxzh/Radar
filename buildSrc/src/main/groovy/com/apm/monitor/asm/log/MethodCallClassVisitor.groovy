package com.apm.monitor.asm.log
...
import com.apm.monitor.util.OpcodesUtils
import com.apm.monitor.util.Utils


class MethodCallClassVisitor extends ClassVisitor {
...
//any reference type. include array.
    private static final Type SLOT_TYPE = Type.getType("LSlot;");//Operand that occupies a slot
    private static final Type DOUBLE_SLOT_TYPE = Type.getType("LSlot2;");
//Operand that occupies two slots
    ...
    MethodCallClassVisitor() {
        super(Opcodes.ASM9)
    }

    MethodCallClassVisitor(ClassVisitor classVisitor) {
    ...
    class MethodCallOptMethodVisitor extends MethodNode {
        ...
        void visitEnd() {
            ...
            while (index >= 0) {
                ...
                if (node.getOpcode() == Opcodes.INVOKESTATIC || node.getOpcode() == Opcodes.INVOKESPECIAL || node.getOpcode() == Opcodes.INVOKEVIRTUAL || node.getOpcode() == Opcodes.INVOKEINTERFACE) {
                    ...
                    if (isOptimizationNeededMethod(methodInsnNode.owner, methodInsnNode.name, methodInsnNode.desc, isStatic)) {
                        Type returnType = Type.getReturnType(methodInsnNode.desc)
                        println("Type.VOID_TYPE == returnType :" + (Type.VOID_TYPE == returnType))
                        println("typeOf(returnType, SLOT_TYPE) :" + (typeOf(returnType, SLOT_TYPE)))
                        println("typeOf(returnType, DOUBLE_SLOT_TYPE) :" + (typeOf(returnType, DOUBLE_SLOT_TYPE)))
                        ...
                        if (needOpt) {
                            ...
                            try {
                                ...
                                if (succeedIndex >= 0) {
                                    ...
                                    println("skip opt2" + generateStackTraceString(index) + " :return " + succeedIndex + "\nparamStack=" + getStackString(mParamsStack));
                                }
                            } catch (Exception e) {
                                println("Exception" + generateStackTraceString(index) + " :" + e.toString() + "\nparamStack=" + getStackString(mParamsStack));
                            } catch (AssertionError error) {
                                println("AssertionError" + generateStackTraceString(index) + " :" + error.toString() + "\nparamStack=" + getStackString(mParamsStack));
                            }
                        } else {
                            println("skip opt1" + generateStackTraceString(index));
                        }
                    }
                }
                index--;
            ...
            if (optimizedIns.size() > 0) {
                ...
                println(Utils.replaceSlash2Dot(mClassName) + "." + mMethodName + stringBuilder.toString());
            }
            if (this.mMv != null) {
                accept(this.mMv);
            }
        ...
        private boolean isOptimizationNeededMethod(String className, String methodName, String desc, boolean isStatic) {
            if (className == "android/util/Log" && (methodName == "v" || methodName == "d" || methodName == "i" || methodName == "w" || methodName == "e")) {
                return true
            }
            return false
        }
                ...
                case Opcodes.AALOAD:
                    if (typeOf(OBJECT_TYPE, type)) {
                        ...
                        if (type == SLOT_TYPE || type == OBJECT_TYPE) {
                            push(OBJECT_TYPE);
                            println("AALOAD=" + type.getDescriptor());
                        } else {
